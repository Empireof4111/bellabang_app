...
Future<void>  myWishList() async {
  wishList = await wishListServices.userWishList(context,newUserId);
  // setState(() {
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              wishList!.isEmpty ? const NoWishListWidget() : SizedBox(
              height: 570,
              child: 
              ListView.builder(
                shrinkWrap: true,
                itemCount: (wishList == null) ? 0 : wishList!.length,  itemBuilder: (context, index){
               return WishListCard(
                productName: wishList![index].product!.name as String, 
                productPrice: wishList![index].product!.price as double,
                productSize: wishList![index].product!.sizes as String, 
                productColor: wishList![index].product!.colors as String,
                productImageLink: wishList![index].product!.images!.split(',')[0].toString(), 
                addToCart: () { 
                  cartBox.put(wishList![index].product!.id, 
                  Addtocartmodel(
                  id: wishList![index].product!.id as int, 
                  cartCurrencyCode: wishList![index].product!.currencyCode as String, 
                  cartServiceCharged: wishList![index].product!.serviceCharge as double, 
                  cartShippingFee: wishList![index].product!.shippingFees as double, 
                  cartImageUrl: wishList![index].product!.images!.split(',')[0].toString(), 
                  productName: wishList![index].product!.name.toString(), 
                  productColor: wishList![index].product!.colors.toString(), 
                  productSize: wishList![index].product!.sizes.toString(), 
                  productQuantity: 1,
                  productPrice: wishList![index].product!.price as double,
                  )
                  );
                  deleteListFunction(wishList![index].info!.id as int);
                  showSnackBar(context, wishList![index].product!.name.toString());
                }, 
                deleteList: (){
                setState(() {
                deleteListFunction(wishList![index].info!.id as int);
                });
                }, currencyType: wishList![index].product!.currencyCode as String,);
              })
            ),
            const SizedBox(height: 10,),
          Visibility(
...
class _WishListCardState extends State<WishListCard> {
  ...
  @override
  Widget build(BuildContext context) {
    double? finaAmount = basedCurrencyConvertion(widget.currencyType, widget.productPrice, newExchangeRates);
    return Padding(
        ...
        child: Padding(
          ...
          child: Row(
            ...
            Container(
                      ...
                      child: Image.network(fit: BoxFit.fill, scale: 1, "$imageUrl${widget.productImageLink}")
                  ...
                  Row(
                    children: [
                      Column(
                        ...
                        children: [
                          Text(widget.productName, style: const TextStyle(
                                  fontWeight: FontWeight.bold,
                                  fontSize: 14,
                                ),),
                             Row(
                               children: [
                                 Text(currencySymbolConveeter(currencyChoosed), style: const TextStyle(
                                      fontWeight: FontWeight.bold,
                                      fontSize: 20, color: Colors.red,
                                    ),),
                                     Text(finaAmount!.toStringAsFixed(2), style: const TextStyle(
                                      fontWeight: FontWeight.bold,
                                      fontSize: 20, color: Colors.red,
                                    ),),
                               ],
                                ...
                                const SizedBox(height: 14,),
                                 Text('Color: ${widget.productColor}', style: const TextStyle(
                                  fontSize: 12,
                                ),), Text('Size: ${widget.productColor}', style: const TextStyle(
                                  fontSize: 12, 
                                ),),
                        ],
                      ),
